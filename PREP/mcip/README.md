...
NACC was developed at by Dr. Patrick C. Campbell and Dr. Youhua Tang, with support from the entire NOAA-ARL research team (Contact:  Patrick.C.Campbell@noaa.gov).  Hereafter the documentation of the MCIP-based NACC code will only be referred to as the NACC.
...
NACC is written in FORTRAN, and it runs on a single processor in a Unix/Linux environment. NACC is driven by a C-shell or K-shell script with several run-time options that are defined through a FORTRAN namelist. It is typical to use NACC to process hourly output fields from the meteorological model for each one-day period.
...
-   `NACC_START [format: YYYY-MM-DD-HH:MM:SS.SSSS]`  
...
-   `NACC_END [format: YYYY-MM-DD-HH:MM:SS.SSSS]`  
...
NACC is compiled with a Makefile. The configuration options in the Makefile include the compiler and compiler flags to use for building the executable. Note that NACC is not a parallelized code, so parallel versions of netCDF and I/O API are not required.  The Makefile is located in the directory with the NACC source code (`$CMAQ_HOME/PREP/mcip/src`). To compile NACC, source the config_cmaq.csh file and invoke the Makefile at the command line:
...
NACCv4.3 and NACCv4.2 were released via the CMASCenter NACC GitHub repository.  Beginning with version 4.5, NACC is released as part of the CMAQ repository.  
* [NACC v4.5 (October 2018)](https://github.com/USEPA/CMAQ/tree/CMAQv5.3.b2_19Oct2018) - bundled under CMAQv5.3beta2 
* [NACC v4.3 (November 2015)](https://github.com/CMASCenter/NACC/tree/4.3) - available on CMASCenter GitHub repository
* [NACC v4.2 (December 2013)](https://github.com/CMASCenter/NACC/tree/4.2) - available on CMASCenter GitHub repository
